...
## MonetdbDriver
## ==============================================
class MonetdbDriver(AbstractDriver):
    DEFAULT_CONFIG = {
    ...
    def __init__(self, ddl):
        super(MonetdbDriver, self).__init__("monetdb", ddl)
    ...
    def makeDefaultConfig(self):
        return MonetdbDriver.DEFAULT_CONFIG
    
    ## ----------------------------------------------
    ## loadConfig
    ## ----------------------------------------------
    def loadConfig(self, config):
        for key in MonetdbDriver.DEFAULT_CONFIG.keys():
    ...
    def loadFromCsv(self):
        ...
        self.conn.autocommit = True
        tables = ['item', 'warehouse', 'new_order', 'orders', 'order_line', 'customer', 'district', 'stock', 'history']
    ...
    def doDelivery(self, params):
        try:
            ...
            for d_id in range(1, constants.DISTRICTS_PER_WAREHOUSE+1):
                ...
                writes.append(q["deleteNewOrder"] % (d_id, w_id, no_o_id))
                #self.cursor.execute(q["updateOrders"] % (o_carrier_id, no_o_id, d_id, w_id))
                writes.append(q["updateOrders"] % (o_carrier_id, no_o_id, d_id, w_id))
                #self.cursor.execute(q["updateOrderLine"] % (ol_delivery_d, no_o_id, d_id, w_id))
                writes.append(q["updateOrderLine"] % (ol_delivery_d, no_o_id, d_id, w_id))
                ...
                writes.append(q["updateCustomer"] % (ol_total, c_id, d_id, w_id))
    ...
    def doNewOrder(self, params):
        try:
            ...
            writes.append(q["incrementNextOrderId"] % (d_next_o_id + 1, d_id, w_id))
            #self.cursor.execute(q["createOrder"] % (d_next_o_id, d_id, w_id, c_id, o_entry_d, o_carrier_id, ol_cnt, all_local))
            writes.append(q["createOrder"] % (d_next_o_id, d_id, w_id, c_id, o_entry_d, o_carrier_id, ol_cnt, all_local))
            #self.cursor.execute(q["createNewOrder"] % (d_next_o_id, d_id, w_id))
            writes.append(q["createNewOrder"] % (d_next_o_id, d_id, w_id))
            ...
            for i in range(len(i_ids)):
                ...
                writes.append(q["updateStock"] % (s_quantity, s_ytd, s_order_cnt, s_remote_cnt, ol_i_id, ol_supply_w_id))
                ...
                writes.append(q["createOrderLine"] % (d_next_o_id, d_id, w_id, ol_number, ol_i_id, ol_supply_w_id, o_entry_d, ol_quantity, ol_amount, s_dist_xx))
    ...
    def doPayment(self, params):
        try:
            ...
            writes.append(q["updateDistrictBalance"] % (h_amount, w_id, d_id))
            ...
            if customer[11] == constants.BAD_CREDIT:
                ...
                writes.append(q["updateBCCustomer"] % (c_balance, c_ytd_payment, c_payment_cnt, c_data, c_w_id, c_d_id, c_id))
                ...
                writes.append(q["updateGCCustomer"] % (c_balance, c_ytd_payment, c_payment_cnt, c_w_id, c_d_id, c_id))
                #print(f'EXEC_update_customer,{time.time() - begin_update_customer}')
            ...
            #print(f'EXEC,{time.time() - begin_exec}')
