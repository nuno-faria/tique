...
SCHEMA = '''
    ...
    CREATE TABLE ORDER_LINE (
        ...
        SORT KEY (OL_W_ID, OL_D_ID, OL_O_ID),
        key (OL_W_ID, OL_D_ID, OL_O_ID)
    );
...
## SinglestoreDriver
## ==============================================
class SinglestoreDriver(AbstractDriver):
    DEFAULT_CONFIG = {
        "driver": ("The odbc driver used", "/usr/lib/x86_64-linux-gnu/libssodbcw.so" ),
        "host": ("The server address", "127.0.0.1"),
        "port": ("The server port", "3306"),
        "database": ("The database used", "tpcc"),
        "username": ("The username", "root"),
        "password": ("The password", "root")
    }
    

    def __init__(self, ddl):
        super(SinglestoreDriver, self).__init__("singlestore", ddl)
    ...
    def makeDefaultConfig(self):
        return SinglestoreDriver.DEFAULT_CONFIG
    
    ## ----------------------------------------------
    ## loadConfig
    ## ----------------------------------------------
    def loadConfig(self, config):
        for key in SinglestoreDriver.DEFAULT_CONFIG.keys():
