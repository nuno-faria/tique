...
TXN_QUERIES = {
    "DELIVERY": {
        "getNewOrder": "SELECT NO_O_ID FROM NEW_ORDER WHERE pk_partial = '%s.%s' LIMIT 1", #
        "deleteNewOrder": "DELETE FROM NEW_ORDER WHERE pk = '%s'", # d_id, w_id, no_o_id
        "getCId": "SELECT O_C_ID FROM ORDERS WHERE pk = '%s'", # no_o_id, d_id, w_id
        "updateOrders": "UPDATE ORDERS SET O_CARRIER_ID = %s WHERE pk = '%s'", # o_carrier_id, no_o_id, d_id, w_id
        "updateOrderLine": "UPDATE ORDER_LINE SET OL_DELIVERY_D = '%s' WHERE pk = '%s'", # o_entry_d, no_o_id, d_id, w_id
        "sumOLAmount": "SELECT SUM(OL_AMOUNT) FROM ORDER_LINE WHERE pk = '%s'", # no_o_id, d_id, w_id
        "updateCustomer": "UPDATE CUSTOMER SET C_BALANCE = C_BALANCE + %s WHERE pk = '%s'", # ol_total, c_id, d_id, w_id
    },

    "NEW_ORDER": {
        "getWarehouseTaxRate": "SELECT W_TAX FROM WAREHOUSE WHERE W_ID = %s", # w_id
        "getDistrict": "SELECT D_TAX, D_NEXT_O_ID FROM DISTRICT WHERE pk = '%s'", # d_id, w_id
        "incrementNextOrderId": "UPDATE DISTRICT SET D_NEXT_O_ID = %s WHERE pk = '%s'", # d_next_o_id, d_id, w_id
        "getCustomer": "SELECT C_DISCOUNT, C_LAST, C_CREDIT FROM CUSTOMER WHERE pk = '%s'", # w_id, d_id, c_id
        "createOrder": "INSERT INTO ORDERS (O_ID, O_D_ID, O_W_ID, O_C_ID, O_ENTRY_D, O_CARRIER_ID, O_OL_CNT, O_ALL_LOCAL, pk) VALUES (%s, %s, %s, %s, '%s', %s, %s, %s, '%s')", # d_next_o_id, d_id, w_id, c_id, o_entry_d, o_carrier_id, o_ol_cnt, o_all_local
        "createNewOrder": "INSERT INTO NEW_ORDER (NO_O_ID, NO_D_ID, NO_W_ID, pk, pk_partial) VALUES (%s, %s, %s, '%s', '%s')", # o_id, d_id, w_id
        "getItemInfo": "SELECT I_PRICE, I_NAME, I_DATA FROM ITEM WHERE I_ID = %s", # ol_i_id
        "getStockInfo": "SELECT S_QUANTITY, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT, S_DIST_%02d FROM STOCK WHERE pk = '%s'", # d_id, ol_i_id, ol_supply_w_id
        "updateStock": "UPDATE STOCK SET S_QUANTITY = %s, S_YTD = %s, S_ORDER_CNT = %s, S_REMOTE_CNT = %s WHERE pk = '%s'", # s_quantity, s_order_cnt, s_remote_cnt, ol_i_id, ol_supply_w_id
        "createOrderLine": "INSERT INTO ORDER_LINE (OL_O_ID, OL_D_ID, OL_W_ID, OL_NUMBER, OL_I_ID, OL_SUPPLY_W_ID, OL_DELIVERY_D, OL_QUANTITY, OL_AMOUNT, OL_DIST_INFO, pk, pk_partial) VALUES (%s, %s, %s, %s, %s, %s, '%s', %s, %s, '%s', '%s', '%s')", # o_id, d_id, w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info        
    },
    
    "ORDER_STATUS": {
        "getCustomerByCustomerId": "SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_BALANCE FROM CUSTOMER WHERE pk = '%s'", # w_id, d_id, c_id
        "getCustomersByLastName": "SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_BALANCE FROM CUSTOMER WHERE pk_last = '%s' ORDER BY C_FIRST", # w_id, d_id, c_last
        "getLastOrder": "SELECT O_ID, O_CARRIER_ID, O_ENTRY_D FROM ORDERS WHERE pk = '%s' ORDER BY O_ID DESC LIMIT 1", # w_id, d_id, c_id
        "getOrderLines": "SELECT OL_SUPPLY_W_ID, OL_I_ID, OL_QUANTITY, OL_AMOUNT, OL_DELIVERY_D FROM ORDER_LINE WHERE pk = '%s'", # w_id, d_id, o_id        
    },
    ...
    "PAYMENT": {
        ...
        "getDistrict": "SELECT D_NAME, D_STREET_1, D_STREET_2, D_CITY, D_STATE, D_ZIP FROM DISTRICT WHERE pk = '%s'", # w_id, d_id
        "updateDistrictBalance": "UPDATE DISTRICT SET D_YTD = D_YTD + %s WHERE pk = '%s'", # h_amount, d_w_id, d_id
        "getCustomerByCustomerId": "SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_DATA FROM CUSTOMER WHERE pk = '%s'", # w_id, d_id, c_id
        "getCustomersByLastName": "SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_DATA FROM CUSTOMER WHERE pk_last = '%s' ORDER BY C_FIRST", # w_id, d_id, c_last
        "updateBCCustomer": "UPDATE CUSTOMER SET C_BALANCE = %s, C_YTD_PAYMENT = %s, C_PAYMENT_CNT = %s, C_DATA = '%s' WHERE pk = '%s'", # c_balance, c_ytd_payment, c_payment_cnt, c_data, c_w_id, c_d_id, c_id
        "updateGCCustomer": "UPDATE CUSTOMER SET C_BALANCE = %s, C_YTD_PAYMENT = %s, C_PAYMENT_CNT = %s WHERE pk = '%s'", # c_balance, c_ytd_payment, c_payment_cnt, c_w_id, c_d_id, c_id
        "insertHistory": "INSERT INTO HISTORY VALUES ('%s', %s, %s, %s, %s, %s, '%s', %s, '%s')",
    },
    
    "STOCK_LEVEL": {
        "getOId": "SELECT D_NEXT_O_ID FROM DISTRICT WHERE pk = '%s'",
        "getStockCount": """
            SELECT COUNT(DISTINCT(OL_I_ID))
            FROM ORDER_LINE, STOCK
            WHERE ORDER_LINE.pk like '%s.%s.%%' -- this is faster than "pk_partial = ..." in native monetdb 
            AND S_W_ID = %s
            AND S_I_ID = OL_I_ID
            AND S_QUANTITY < %s
            and OL_O_ID < %s
                AND OL_O_ID >= %s
        """,
        # this query works better when stock level is running alone,
        # but with all procedures it results in worse performance
        #"getStockCount": """
        #    SELECT COUNT(DISTINCT(OL_I_ID)) FROM (
        #        SELECT OL_O_ID + 1 - 1 AS OL_O_ID, OL_I_ID -- + 1 - 1 to force the OL_O_ID filter to be applied at the end, since it executes faster
        #        FROM ORDER_LINE, STOCK
        #        WHERE ORDER_LINE.pk_partial = '%s.%s'
        #        AND S_W_ID = %s
        #        AND S_I_ID = OL_I_ID
        #        AND S_QUANTITY < %s
        #    ) t
...
SCHEMA = '''
    ...
    CREATE INDEX NATION_IDX ON NATION (n_nationkey);
    ...
    CREATE INDEX SUPPLIER_IDX ON SUPPLIER (su_suppkey);
'''


## ==============================================
## MonetdbsinglekeyDriver
## ==============================================
class MonetdbsinglekeyDriver(AbstractDriver):
    DEFAULT_CONFIG = {
    ...
    def __init__(self, ddl):
        super(MonetdbsinglekeyDriver, self).__init__("monetdb", ddl)
    ...
    def makeDefaultConfig(self):
        return MonetdbsinglekeyDriver.DEFAULT_CONFIG
    
    ## ----------------------------------------------
    ## loadConfig
    ## ----------------------------------------------
    def loadConfig(self, config):
        for key in MonetdbsinglekeyDriver.DEFAULT_CONFIG.keys():
    ...
    def doDelivery(self, params):
        try:
            ...
            for d_id in range(1, constants.DISTRICTS_PER_WAREHOUSE+1):
                self.cursor.execute(q["getNewOrder"] % (w_id, d_id))
                ...
                self.cursor.execute(q["getCId"] % (f'{w_id}.{d_id}.{no_o_id}'))
                c_id = self.cursor.fetchone()[0]
                
                self.cursor.execute(q["sumOLAmount"] % (f'{w_id}.{d_id}.{no_o_id}'))
                ol_total = self.cursor.fetchone()[0]

                #self.cursor.execute(q["deleteNewOrder"] % (f'{w_id}.{d_id}.{no_o_id}'))
                writes.append(q["deleteNewOrder"] % (f'{w_id}.{d_id}.{no_o_id}'))
                #self.cursor.execute(q["updateOrders"] % (o_carrier_id, f'{w_id}.{d_id}.{no_o_id}'))
                writes.append(q["updateOrders"] % (o_carrier_id, f'{w_id}.{d_id}.{no_o_id}'))
                #self.cursor.execute(q["updateOrderLine"] % (ol_delivery_d, f'{w_id}.{d_id}.{no_o_id}'))
                writes.append(q["updateOrderLine"] % (ol_delivery_d, f'{w_id}.{d_id}.{no_o_id}'))
                ...
                writes.append(q["updateCustomer"] % (ol_total, f'{w_id}.{d_id}.{c_id}'))

                result.append((d_id, no_o_id))
            ## FOR

            if writes != []:
                # minimal pipe optimizer is slow with the updates on the customer table
                writes.insert(0, "set optimizer = 'default_pipe'")
                # revert back to the original optimizer for the other transactions
                writes.append("set optimizer = 'minimal_pipe'")
                self.cursor.execute(';'.join(writes))


            #print(f'EXEC_DELIVERY,{time.time() - begin_exec}')
    ...
    def doNewOrder(self, params):
        try:
            ...
            self.cursor.execute(q["getDistrict"] % (f'{w_id}.{d_id}'))
            district_info = self.cursor.fetchone()
            d_tax = district_info[0]
            d_next_o_id = district_info[1]
            
            #self.cursor.execute(q["getCustomer"] % (f'{w_id}.{d_id}.{c_id}'))
            self.cursor.execute(q["getCustomer"] % (f'{w_id}.{d_id}.{c_id}'))
            ...
            #self.cursor.execute(q["incrementNextOrderId"] % (d_next_o_id + 1, f'{w_id}.{d_id}'))
            writes.append(q["incrementNextOrderId"] % (d_next_o_id + 1, f'{w_id}.{d_id}'))
            #self.cursor.execute(q["createOrder"] % (d_next_o_id, d_id, w_id, c_id, o_entry_d, o_carrier_id, ol_cnt, all_local, f'{w_id}.{d_id},{d_next_o_id}'))
            writes.append(q["createOrder"] % (d_next_o_id, d_id, w_id, c_id, o_entry_d, o_carrier_id, ol_cnt, all_local, f'{w_id}.{d_id}.{d_next_o_id}'))
            #self.cursor.execute(q["createNewOrder"] % (d_next_o_id, d_id, w_id, f'{w_id}.{d_id}.{d_next_o_id}'))
            writes.append(q["createNewOrder"] % (d_next_o_id, d_id, w_id, f'{w_id}.{d_id}.{d_next_o_id}', f'{w_id}.{d_id}'))
            ...
            for i in range(len(i_ids)):
                ...
                self.cursor.execute(q["getStockInfo"] % (d_id, f'{ol_supply_w_id}.{ol_i_id}'))
                ...
                writes.append(q["updateStock"] % (s_quantity, s_ytd, s_order_cnt, s_remote_cnt, f'{ol_supply_w_id}.{ol_i_id}'))
                ...
                writes.append(q["createOrderLine"] % (d_next_o_id, d_id, w_id, ol_number, ol_i_id, ol_supply_w_id, o_entry_d, ol_quantity, ol_amount, s_dist_xx, f'{w_id}.{d_id}.{d_next_o_id}', f'{w_id}.{d_id}'))
            ...
            #print(f'EXEC_NEW_ORDER,{time.time() - begin_exec}')
    ...
    def doOrderStatus(self, params):
        ...
        if c_id != None:
            #self.cursor.execute(q["getCustomerByCustomerId"] % (f'{w_id}.{d_id}.{c_id}'))
            self.cursor.execute(q["getCustomerByCustomerId"] % (f'{w_id}.{d_id}.{c_id}'))
            customer = self.cursor.fetchone()
        else:
            # Get the midpoint customer's id
            #self.cursor.execute(q["getCustomersByLastName"] % (f'{w_id}.{d_id}.{c_last}'))
            self.cursor.execute(q["getCustomersByLastName"] % (f'{w_id}.{d_id}.{c_last}'))
        ...
        self.cursor.execute(q["getLastOrder"] % (f'{w_id}.{d_id}.{c_id}'))
        order = self.cursor.fetchone()
        if order:
            self.cursor.execute("set optimizer = 'default_pipe'; " + q["getOrderLines"] % (f'{w_id}.{d_id}.{order[0]}') + "; set optimizer = 'minimal_pipe';")
        ...
        #print(f'EXEC_ORDER_STATUS,{time.time() - begin_exec}')
    ...
    def doPayment(self, params):
        try:
            ...
            if c_id != None:
                ...
                #self.cursor.execute(q["getCustomerByCustomerId"] % (f'{w_id}.{d_id}.{c_id}'))
                self.cursor.execute("set optimizer = 'default_pipe'; " + q["getCustomerByCustomerId"] % (f'{w_id}.{d_id}.{c_id}') + "; set optimizer = 'minimal_pipe'; ") # generates a better plan for multiple clients
                customer = self.cursor.fetchone()
                #print(f'EXEC_read_customer,{time.time() - begin_read_customer}')
            else:
                # Get the midpoint customer's id
                #begin_read_customer = time.time()
                #self.cursor.execute(q["getCustomersByLastName"] % (f'{w_id}.{d_id}.{c_last}'))
            ...
            #print(f'EXEC_read_warehouse,{time.time() - begin_read_warehouse}')
            
            #begin_read_district = time.time()
            self.cursor.execute(q["getDistrict"] % (f'{w_id}.{d_id}'))
            district = self.cursor.fetchone()
            #print(f'EXEC_read_district,{time.time() - begin_read_district}')
            
            #begin_update_warehouse = time.time()
            #self.cursor.execute(q["updateWarehouseBalance"], [h_amount, w_id])
            writes.append(q["updateWarehouseBalance"] % (h_amount, w_id))
            #print(f'EXEC_update_warehouse,{time.time() - begin_update_warehouse}')
            #begin_update_district = time.time()
            #self.cursor.execute(q["updateDistrictBalance"], [h_amount, f'{w_id}.{d_id}'])
            writes.append(q["updateDistrictBalance"] % (h_amount, f'{w_id}.{d_id}'))
            #print(f'EXEC_update_district,{time.time() - begin_update_district}')
            ...
            if customer[11] == constants.BAD_CREDIT:
                ...
                #begin_update_customer = time.time()
                #self.cursor.execute(q["updateBCCustomer"], [c_balance, c_ytd_payment, c_payment_cnt, c_data, f'{c_w_id}.{c_d_id}.{c_id}'])
                writes.append(q["updateBCCustomer"] % (c_balance, c_ytd_payment, c_payment_cnt, c_data, f'{c_w_id}.{c_d_id}.{c_id}'))
                #print(f'EXEC_update_customer,{time.time() - begin_update_customer}')
            else:
                c_data = ""
                #begin_update_customer = time.time()
                #self.cursor.execute(q["updateGCCustomer"], [c_balance, c_ytd_payment, c_payment_cnt, f'{c_w_id}.{c_d_id}.{c_id}'])
                writes.append(q["updateGCCustomer"] % (c_balance, c_ytd_payment, c_payment_cnt, f'{c_w_id}.{c_d_id}.{c_id}'))
                #print(f'EXEC_update_customer,{time.time() - begin_update_customer}')
            ...
            #begin_insert_hist = time.time()
            #self.cursor.execute(q["insertHistory"], [h_id, c_id, c_d_id, c_w_id, d_id, w_id, h_date, h_amount, h_data])
            writes.append(q["insertHistory"] % (h_id, c_id, c_d_id, c_w_id, d_id, w_id, h_date, h_amount, h_data))
            #print(f'EXEC_insert_history,{time.time() - begin_insert_hist}')

            # execute writes
            self.cursor.execute(';'.join(writes))

            #print(f'EXEC_PAYMENT,{time.time() - begin_exec}')
            
            #begin_commit = time.time()
            self.cursor.execute('commit')
            #print(f'COMMIT,{time.time() - begin_commit}')
    ...
    def doStockLevel(self, params):
        ...
        self.cursor.execute(q["getOId"] % (f'{w_id}.{d_id}'))
        result = self.cursor.fetchone()
        assert result
        o_id = result[0]
        
        self.cursor.execute(q["getStockCount"] % (w_id, d_id, w_id, threshold, o_id, (o_id - 20)))
        result = self.cursor.fetchone()

        #print(f'EXEC_STOCK_LEVEL,{time.time() - begin_exec}')
